...
namespace Ava {
	...
	{
		AVA_CORE_ERROR("GLFW Error ({0}): {1}", error, description);
	}
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		AVA_CORE_INFO("Creating window {0} ({1}, {2})", props.Title, props.width, props.height);

		if (!s_GLFWInitialized)
		{
			int success = glfwInit();
			AVA_CORE_ASSERT(success, "Could not intialize GLFW!");
